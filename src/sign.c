...
int PREFIX_NAME(crypto_sign)(
  ...
  const unsigned char *sk, UINT *smT)
{
    *smlen=mlen+CRYPTO_BYTES;
    memcpy(sm+CRYPTO_BYTES,m,(size_t)mlen);
    return signHFE(sm,m,(size_t)mlen,(UINT*)sk, (UINT*)smT);
}
...
int PREFIX_NAME(crypto_sign_open)(
  ...
  const unsigned char *pk, UINT* S0_Test)
{
    int result;
    *mlen=smlen-CRYPTO_BYTES;
    result=sign_openHFE(sm+CRYPTO_BYTES,(size_t)(*mlen),sm,pk,S0_Test);
