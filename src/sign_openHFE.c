#include <stdio.h>      // Murat was here: After disabling KAT_INT definition, we need to add stdio. for printf
                                         ...
                                         void (*eval_pk)(), UINT* S0_Test)
{
    ...
    /*MD: Check if S0 equals to zero */
//    UINT S0_Test[SIZE_SIGN_UNCOMPRESSED-SIZE_SALT_WORD]={0};
    add_gf2m(S0_Test,Si,D);
    /*
    printf("S0_Test: ");
    for(i = 0; i < NB_WORD_GF2nv; i++){
        printf("%llx", S0_Test[i]);
    }
    printf("\n"); */
...
int PREFIX_NAME(sign_openHFE)(const unsigned char* m, size_t len,
                              const unsigned char* sm8, const uint8_t* pk, UINT* S0_Test)
{
//    printf("Find the verification function.\n");  \\ Murat was here --> Follow this line
    int ret;
...
//    printf("test1;\n"); //Murat was here --> Follow this line
        ret=sign_openHFE_huncomp_pk(m,len,sm8,pk,(uint8_t*)pk_tmp,
                                    evalMQShybrid8_nocst_gf2_m, S0_Test);
    #else
//    printf("test2;\n"); //Murat was here
